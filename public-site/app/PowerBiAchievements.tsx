...
export default function PowerBiAchievements({ sections, onOpen }: { sections: Section[]; onOpen: (section: Section) => void }) {
  ...
  return <section className="min-h-[calc(100vh-4rem)] bg-slate-950 pb-16 text-white">
    <div className="relative overflow-hidden border-b border-white/10" style={{ backgroundImage: 'linear-gradient(90deg, rgba(2,6,23,.97), rgba(2,6,23,.72)), url(/api/uploads/bigdata-bg.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[1.2fr_.8fr] lg:items-center lg:py-20">
        <div><p className="inline-flex rounded-full border border-yellow-300/25 bg-yellow-300/10 px-3 py-1 text-xs font-bold uppercase tracking-[.18em] text-yellow-200">Trayectoria Power BI</p><h1 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">Cursos y logros que convierten datos en <span className="text-cyan-300">información útil.</span></h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">Este espacio reúne exclusivamente las imágenes y enlaces publicados en el módulo Logros Curso PowerBI.</p></div>
